...
admins = [1042304148, 868006823, 1796733875, 7498229612] # 
TOKEN = '6996297016:AAFaYQFX_BxlwEaAvGZmgaJQQ4ubtoRi68Q'
# JIJI
...
handler = logging.FileHandler('Jiji.log', encoding='utf-8')
    ...
    for v1, count in flavors_and_counts:
        ...
        if existing_entry is None:
            ...
            if sql.fetchone() is None:
                ...
                await send_new_post(name, type, x1, x2, v1, buy, sell, message_id, message)
                await zakrep_edit(message)
            else:
                message_count += 1
                await message.answer(f'Пост {name} изменён. ({message_count})')
                await edit_post(name, type, x1, x2, v1, buy, sell, message_id, message)
        else:
...
async def send_new_post(name, type, x1, x2, v1, buy, sell, message_id, message):
    ...
    try:
        ...
        for flavor in flavors_list:
            ...
            db.commit()
    db_file_path = 'Jiji.db'
    await message.answer_document(FSInputFile(db_file_path))

async def edit_post(name, type, x1, x2, v1, buy, sell, message_id, message):
    sql.execute(f'''SELECT mess_id from JIJI WHERE name=(?) AND type=(?) AND x1=(?) AND x2=(?) AND sell=(?) ''', 
                (name, type, x1, x2, sell))
    message_id = sql.fetchone()[0]
